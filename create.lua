...
local function conver(str)
    ...
    local tmp = str:split("|")
    for v = 1, #tmp do
        if tmp[v]:lower() == "sn" then tmp[v] = hex and (mobile.sn():toHex()) or mobile.sn() end
        if tmp[v]:lower() == "imei" then tmp[v] = hex and (mobile.imei():toHex()) or mobile.imei() end
        if tmp[v]:lower() == "muid" then tmp[v] = hex and (mobile.muid():toHex()) or mobile.muid() end
        if tmp[v]:lower() == "imsi" then tmp[v] = hex and (mobile.imsi():toHex()) or mobile.imsi() end
        if tmp[v]:lower() == "iccid" then tmp[v] = hex and (mobile.iccid():toHex()) or mobile.iccid() end
        if tmp[v]:lower() == "csq" then tmp[v] = hex and string.format("%02X", mobile.rssi()) or tostring(mobile.rssi()) end    
    end
...
local function loginMsg(str)
    ...
    elseif tonumber(str) == 1 then
        return json.encode({csq = mobile.rssi(), imei = mobile.imei(), iccid = mobile.iccid(), ver = _G.VERSION})
...
function socketrecv(timeout,msg,msgNoResume,prot,socketOutput,socketInput) 
    -- local r,data,param=sys.waitUntil(msg,timeout)
    local co=coroutine.running()
    if msg then
        subMessage=function (data)
            if not msgNoResume then
                if data then table.insert(socketOutput,data) end
            end
            coroutine.resume(co,0xAA)
        end
        sys.subscribe(msg,subMessage)
    end
    if msg and #socketOutput>0 then
        sys.publish(msg,false)
    end
    if #socketInput==0 then
        if timeout and timeout>0 then
            local r,s=sys.wait(timeout)
            if r==nil then
                return "timeout"
            elseif r==0xAA then
                local dat=table.concat(socketOutput)
                socketOutput={}
            else
                log.info("走到这里来了3")
                return r, s
...
local function tcpTask(cid, pios, reg, convert, passon, upprot, dwprot, prot, ping, timeout, addr, port, uid, gap, report, intervalTime, ssl, login)
    ...
    while true do
        ...
        log.info("prot",prot)
        log.info("ping",addr)
            local isUdp=prot=="TCP"and false or true
            local isSsl=ssl and true or false
        ...
        if result then
            ...
            -- 登陆报文
            datalink[cid] = true
            if login or loginMsg(reg) then socket.tx(netc,login or loginMsg(reg)) end
            ...
            while true do
                ...
                local subMessage=function (data)
                        if data then table.insert(outputSocket,data)
                ...
                if param == false then
                    local result, param = libnet.tx(d1Name,nil, netc, conver(ping))
                ...
                local is_err, param, _, _ = socket.rx(netc, rx_buff)
                if is_err then
                ...
                if rx_buff:used() > 0 then
                    ...
                    local data=rx_buff:toStr(0,#rx_buff)
                    if data:sub(1, 5) == "rrpc," or data:sub(1, 7) == "config," then
                        ...
                        if not socket.tx(netc,msg) then break end
                ...
                if tx_buff:used() > 0 then
                    local data=tx_buff:toStr(0,#tx_buff)
                    if convert == 1 then -- 转换为Hex String 报文
                        local result, param = libnet.tx(d1Name,nil, netc, data:toHex())
                        if not result then 
                            if passon then 
                    ...
                    elseif convert == 0 and upprotFnc then -- 转换为用户自定义报文
                        ...
                        if not res or not msg then
                            ...
                            local result, param = libnet.tx(d1Name,nil, netc, res and msg or data)
                            if not result then if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_ERROR\r\n") end break end
                        end
                    else -- 默认不转换
                        local result, param = libnet.tx(d1Name,nil, netc, data)
                        if not result then if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_ERROR\r\n") end break end
                    end
    ...
    if #topics == 1 and (not addImei or addImei == "") then
        ...
        for i = 1, #topics, 2 do
            ...
            for v = 1, #tmp do
                ...
                if tmp[v]:lower() == "imsi" then tmp[v] = mobile.imsi() end
                if tmp[v]:lower() == "iccid" then tmp[v] = mobile.iccid() end
                if tmp[v]:lower() == "productid" or tmp[v]:lower()=="{pid}" then tmp[v] = ProductKey end
                if tmp[v]:lower() == "SN" then tmp[v] = hex and (mobile.sn():toHex()) or mobile.sn() end
                ...
                if tmp[v]:lower() == "devicename" or tmp[v]:lower() == "${devicename}" or tmp[v]:lower() == "${yourdevicename}" or tmp[v]:lower()=="{device-name}"then tmp[v] = deviceName end
            end
...
local function mqttTask(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, addr, port, usr, pwd, cleansession, sub, pub, qos, retain, uid,clientID, addImei, ssl, will,idAddImei,prTopic,cert)
    ...
    local dwprotFnc = dwprot and dwprot[cid] and dwprot[cid] ~= "" and loadstring(dwprot[cid]:match("function(.+)end"))
    local upprotFnc = upprot and upprot[cid] and upprot[cid] ~= "" and loadstring(upprot[cid]:match("function(.+)end"))
    if not will or will == "" then will = nil else will = {qos = 1, retain = 0, topic = will, payload = mobile.imei()} end
    ...
    while true do
        ...
        if mqttc:ready() and conres then
            ...
            log.info("pub1",pub[1])
            log.info("qos1",qos)
            -- sub="$thing/down/property/U0VQYMYVW6/a5"
            -- qos=1
            ...
            log.info("A的",a)
               ...
               -- if loginMsg(reg) then mqttc:publish(sub, "hello,server", 1) end
                ...
                while true do
                    ...
                    local ret, topic, data, payload = sys.waitUntil("NET_SENT_RDY_" .. (passon and cid or uid), (timeout or 180)*1000)
                    log.info("RET",ret,topic,data,payload)
                    if ret and topic~="recv" then
                        if convert == 1 then -- 转换为Hex String 报文
                            if not mqttc:publish(pub[1], (topic:toHex()), tonumber(pub[2]) or qos, retain) then if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_ERROR\r\n") end break end
                        elseif convert == 0 and upprotFnc then -- 转换为用户自定义报文
                            ...
                            if not res or not msg then
                                ...
                                if not mqttc:publish(pub_topic, res and msg or topic, tonumber(pub[index + 1]) or qos, retain) then if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_ERROR\r\n") end break end
                            end
                        else
                            local pub_topic = (pub[1]:sub(-1, -1) == "+" and messageId) and pub[1]:sub(1, -2) .. messageId or pub[1]
                            log.info("-----发布的主题:", pub_topic)
                            if not mqttc:publish(pub_topic, topic, tonumber(pub[2]) or qos, retain) then if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_ERROR\r\n") end break end
                        end
                        messageId = false
                        if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_OK\r\n") end
                    ...
                    elseif ret and topic=="recv" then
                        ...
                        elseif convert == 0 and dwprotFnc then -- 转换用户自定义报文
                            ...
                            if not res or not msg then
                                ...
                                if prTopic=="1" then
                                    sys.publish("UART_SENT_RDY_" .. uid, uid, res and ("[+MSUB:"..data..","..#msg..","..msg.."]")or ("[+MSUB:"..data..","..#payload..","..payload.."]"))
                                else
                            ...
                            if prTopic=="1" then
                                ...
                                sys.publish("UART_SENT_RDY_" .. uid, uid, ("[+MSUB:"..data..","..#payload..","..payload.."]"))
                            else
...
---------------------------------------------------------- OneNet 云服务器 ----------------------------------------------------------
-- onenet新版 mqtt 协议支持
local function oneNet_mqtt(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, addr, port, productId, productSecret, deviceName,sub,pub,cleansession, qos, retain, uid)
    cid, keepAlive, timeout, uid = tonumber(cid) or 1, tonumber(keepAlive) or 300, tonumber(timeout), tonumber(uid)
    cleansession, qos, retain= tonumber(cleansession) or 0, tonumber(qos) or 0, tonumber(retain) or 0
    if timeout then sys.timerLoopStart(sys.publish, timeout * 1000, "AUTO_SAMPL_" .. uid) end
    ...
    mqttTask(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, addr, port, username, password, cleansession, sub, pub, qos, retain, uid,clinentId, addImei, ssl, will,"1")
...
local function getOneSecret(RegionId, ProductKey, ProductSecret)
    ...
    for i = 1, 3 do
        local code, head, body = http.request("POST", "https://iot-auth." .. RegionId .. ".aliyuncs.com/auth/register/device",nil,body)
...
local function aliyunOmok(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, RegionId, ProductKey, deviceSecret, deviceName, ver, cleansession, qos, uid, sub, pub)
    ...
    local dwprotFnc = dwprot and dwprot[cid] and dwprot[cid] ~= "" and loadstring(dwprot[cid]:match("function(.+)end"))
    local upprotFnc = upprot and upprot[cid] and upprot[cid] ~= "" and loadstring(upprot[cid]:match("function(.+)end"))
    mqttTask(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, addr, port, usr, pwd, cleansession, sub, pub, qos, retain, uid,clientID, "addImei", ssl, will,"1")
...
function txiot(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, Region, ProductId, SecretId, SecretKey, sub, pub, cleansession, qos, uid)
    if not io.exists("/qqiot.dat") then
        ...
        local data = {ProductId = ProductId, DeviceName = mobile.imei()}
...
function dev_txiotnew(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, Region, deviceName, ProductId, ProductSecret, sub, pub, cleansession, qos, uid)
     enrol_end = false
    if not io.exists("/qqiot.dat") then
        ...
        http.request("POST","https://ap-guangzhou.gateway.tencentdevices.com/register/dev",{["Content-Type"]="application/json; charset=UTF-8"},tx_body_json)
        ...
        while not enrol_end do sys.wait(100) end 
    end
    if not io.exists("/qqiot.dat") then
        log.warn("腾讯云设备注册失败或不存在设备信息!")
        return
    ...
    local payload = json.decode(crypto.aes_decrypt("CBC","ZERO",crypto.base64_decode(dat.payload),string.sub(ProductSecret,1,16),"0000000000000000"))
    local pwd
    log.info("CLIENTID",clientID)
    log.info("user",usr)
    ...
    log.info("pub",pub)
    log.info("qos",qos)
    mqttTask(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, addr, port, usr, pwd, cleansession, sub, pub, qos, retain, uid,clientID, addImei, ssl, will,"1",cert)
            ...
            sys.taskInit(function(cid, convert, passon, upprot, dwprot, uid, method, url, timeout, way, dtype, basic, headers, iscode, ishead, isbody)
                ...
                while true do
                    ...
                    if result and msg then
                        ...
                        if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_OK\r\n") end
                        local code, head, body = dtulib.request(method:upper(), url,timeout*1000,way == 0 and msg or nil, way == 1 and msg or nil, dtype, basic, headers)
                        local headstr = ""
